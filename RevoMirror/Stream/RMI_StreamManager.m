...
@implementation RMI_StreamManager {
    RMI_StreamConfiguration* _config;
...
- (id) initWithConfig:(RMI_StreamConfiguration*)config renderView:(UIView*)view connectionCallbacks:(id<ConnectionCallbacks>)callbacks {
    self = [super init];
    _config = config;
    _renderView = view;
    _callbacks = callbacks;
    _config.riKey = [RMI_Tools randomBytes:16];
...
- (void)main {
    ...
    if (![serverInfoResp isStatusOk]) {
        ...
        [_callbacks launchFailed:RMI_MirrorStr_FailedToPC()];
        return;
    }
    
    if (![pairStatus isEqualToString:@"1"]) {
        // Not paired
        [_callbacks launchFailed:RMI_MirrorStr_DeviceNotPaired()];
        return;
    }
    ...
    if ((_config.width > 4096 || _config.height > 4096) && [serverState containsString:@"MJOLNIR"]) {
        ...
        if (codecSupport == nil || !([codecSupport intValue] & 0x200)) {
            [_callbacks launchFailed:RMI_MirrorStr_DoesnotSupport4K()];
            return;
        }
    }
...
- (BOOL) launchApp:(HttpManager*)hMan receiveSessionUrl:(NSString**)sessionUrl {
    ...
    if (![launchResp isStatusOk]) {
        ...
        [_callbacks launchFailed:RMI_MirrorStr_FailedLaunchApp()];
...
- (BOOL) resumeApp:(HttpManager*)hMan receiveSessionUrl:(NSString**)sessionUrl {
    ...
    if (![resumeResp isStatusOk]) {
        ...
        [_callbacks launchFailed:RMI_MirrorStr_FailedResumeApp()];
