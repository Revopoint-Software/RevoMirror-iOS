...
- (void) initiatePairWithPin:(NSString*)PIN forServerMajorVersion:(int)serverMajorVersion withState:(NSString*)state {
    ...
    NSData* salt = [RMI_Tools randomBytes:16];
    ...
    NSData* derCertBytes = [RMI_CryptoManager pemToDer:[RMI_Tools hexToBytes:plainCert]];
    [_httpManager setServerCert:derCertBytes];
    
    RMI_CryptoManager* cryptoMan = [[RMI_CryptoManager alloc] init];
    NSData* aesKey;
    ...
    NSData* randomChallenge = [RMI_Tools randomBytes:16];
    ...
    NSData* encServerChallengeResp = [RMI_Tools hexToBytes:[challengeResp getStringTag:@"challengeresponse"]];
    ...
    NSData* clientSecret = [RMI_Tools randomBytes:16];
    NSData* challengeRespHashInput = [self concatData:[self concatData:serverChallenge with:[RMI_CryptoManager getSignatureFromCert:_clientCert]] with:clientSecret];
    ...
    NSData* serverSecretResp = [RMI_Tools hexToBytes:[secretResp getStringTag:@"pairingsecret"]];
    ...
    if (![cryptoMan verifySignature:serverSecret withSignature:serverSignature andCert:[RMI_Tools hexToBytes:plainCert]]) {
        [self finishPairing:bgId forResponse:secretResp withFallbackError:@"Server certificate invalid"];
        return;
    }
    
    NSData* serverChallengeRespHashInput = [self concatData:[self concatData:randomChallenge with:[RMI_CryptoManager getSignatureFromCert:[RMI_Tools hexToBytes:plainCert]]] with:serverSecret];
    ...
    NSData* clientPairingSecret = [self concatData:clientSecret with:[cryptoMan signData:clientSecret withKey:[RMI_CryptoManager readKeyFromFile]]];
    HttpResponse* clientSecretResp = [[HttpResponse alloc] init];
    [_httpManager executeRequestSynchronously:[HttpRequest requestForResponse:clientSecretResp withUrlRequest:[_httpManager newClientSecretRespRequest:[RMI_Tools bytesToHex:clientPairingSecret]]]];
