...
static const int staticPorts[numStaticPorts] = {
    ...
    47009, // Port opened by RevoMirror Internet Hosting Tool for WoL (non-privileged port)
};
...
+ (void) wakeHost:(RMI_TemporaryHost*)host {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        NSString* rawAddress = [RMI_Tools addressPortStringToAddress:address];
        unsigned short basePort = [RMI_Tools addressPortStringToPort:address];
...
+ (NSData*) createPayload:(RMI_TemporaryHost*)host {
...
+ (NSData*) macStringToBytes:(NSString*)mac {
    ...
    return [RMI_Tools hexToBytes:macString];
}
