...
@implementation HttpManager {
    ...
    RMI_TemporaryHost *_host; // May be nil
...
- (id) initWithAddress:(NSString*) hostAddressPortString httpsPort:(unsigned short)httpsPort serverCert:(NSData*) serverCert {
    self = [super init];
    // Use the same UID for all RevoMirror clients to allow them
    // quit games started on another RevoMirror client.
    _uniqueId = @"0123456789ABCDEF";
    _deviceName = deviceName;
    _serverCert = serverCert;
    
    NSString* address = [RMI_Tools addressPortStringToAddress:hostAddressPortString];
    unsigned short port = [RMI_Tools addressPortStringToPort:hostAddressPortString];
...
- (BOOL) ensureHttpsUrlPopulated:(bool)fastFail {
    if (!_baseHTTPSURL) {
        ...
        if (_host && _host.httpsPort != 0) {
            ...
            RMI_TemporaryHost* dummyHost = [[RMI_TemporaryHost alloc] init];
...
- (void) executeRequestSynchronously:(HttpRequest*)request {
    ...
    if (!request.request) {
        if (request.response) {
            ...
            request.response.statusMessage = RMI_MirrorStr_HostUnreachable();
        }
...
- (NSURLRequest*) newLaunchOrResumeRequest:(NSString*)verb config:(RMI_StreamConfiguration*)config {
    ...
    NSString* urlString = [NSString stringWithFormat:@"%@/%@?uniqueid=%@&appid=%@&mode=%dx%dx%d&additionalStates=1&sops=%d&rikey=%@&rikeyid=%d%@&localAudioPlayMode=%d&surroundAudioInfo=%d&remoteControllersBitmap=%d&gcmap=%d&gcpersist=%d%s",
                           ...
                           [RMI_Tools bytesToHex:config.riKey], config.riKeyId,
...
- (SecIdentityRef)getClientCertificate {
    ...
    CFDataRef p12Data = (__bridge CFDataRef)[RMI_CryptoManager readP12FromFile];
