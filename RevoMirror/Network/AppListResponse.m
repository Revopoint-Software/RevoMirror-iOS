...
- (void) parseData {
    ...
    while (node != NULL) {
        ...
        if (!xmlStrcmp(node->name, (xmlChar*)TAG_APP)) {
            ...
            if (appId != nil) {
                RMI_TemporaryApp* app = [[RMI_TemporaryApp alloc] init];
                app.name = appName;
                app.id = appId;
    ...
    RMI_TemporaryApp* officialSteamApp = nil;
    RMI_TemporaryApp* manuallyAddedSteamApp = nil;
    for (RMI_TemporaryApp* app in _appList) {
