...
@implementation RMI_TemporaryHost
...
- (id) initFromHost:(Host*)host {
    ...
    if (![self.ipv6Address containsString:@"["]) {
        self.ipv6Address = [RMI_Tools addressAndPortToAddressPortString:self.ipv6Address port:47989];
    }
    ...
    for (App* app in host.appList) {
        RMI_TemporaryApp *tempApp = [[RMI_TemporaryApp alloc] initFromApp:app withTempHost:self];
...
- (NSComparisonResult)compareName:(RMI_TemporaryHost *)other {
