...
@implementation RMI_CryptoManager
...
+ (NSData*) readCertFromFile {
    if (cert == nil) {
        cert = [RMI_CryptoManager readCryptoObject:@"client.crt"];
    }
    return cert;
}
...
+ (NSData*) readP12FromFile {
    if (p12 == nil) {
        p12 = [RMI_CryptoManager readCryptoObject:@"client.p12"];
    }
    return p12;
}
...
+ (NSData*) readKeyFromFile {
    if (key == nil) {
        key = [RMI_CryptoManager readCryptoObject:@"client.key"];
    }
    return key;
}

+ (bool) keyPairExists {
    bool keyFileExists = [RMI_CryptoManager readCryptoObject:@"client.key"] != nil;
    bool p12FileExists = [RMI_CryptoManager readCryptoObject:@"client.p12"] != nil;
    bool certFileExists = [RMI_CryptoManager readCryptoObject:@"client.crt"] != nil;
...
+ (void) generateKeyPairUsingSSL {
    ...
    dispatch_once(&pred, ^{
        if (![RMI_CryptoManager keyPairExists]) {
            Log(LOG_I, @"Generating Certificate... ");
            CertKeyPair certKeyPair = generateCertKeyPair();
            
            NSData* certData = [RMI_CryptoManager getCertFromCertKeyPair:&certKeyPair];
            NSData* p12Data = [RMI_CryptoManager getP12FromCertKeyPair:&certKeyPair];
            NSData* keyData = [RMI_CryptoManager getKeyFromCertKeyPair:&certKeyPair];
            
            freeCertKeyPair(certKeyPair);
            
            [RMI_CryptoManager writeCryptoObject:@"client.crt" data:certData];
            [RMI_CryptoManager writeCryptoObject:@"client.p12" data:p12Data];
            [RMI_CryptoManager writeCryptoObject:@"client.key" data:keyData];
