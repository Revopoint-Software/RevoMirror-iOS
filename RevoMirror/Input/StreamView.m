...
- (void) setupStreamView:(ControllerSupport*)controllerSupport
    ...
    RMI_TemporarySettings* settings = [[[RMI_DataManager alloc] init] getSettings];
...
- (uint16_t)getRotationFromAzimuthAngle:(float)azimuthAngle {
    // iOS reports azimuth of 0 when the stylus is pointing west, but RevoMirror expects
    // rotation of 0 to mean the stylus is pointing north. Rotate the azimuth angle
    // clockwise by 90 degrees to convert from iOS to RevoMirror rotation conventions.
...
- (uint8_t)getTiltFromAltitudeAngle:(float)altitudeAngle {
    ...
    // while RevoMirror expects a tilt of 0 when the stylus is perpendicular to the surface.
    // Subtract the tilt angle from 90 to convert from iOS to RevoMirror tilt conventions.
